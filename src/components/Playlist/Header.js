    ...
    render() {
        ...
        return (
            <Animated.View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: Animated.color(0, 0, 0, background), marginTop: StatusBar.currentHeight, width: Dimensions.get('screen').width, padding: 10, zIndex: 99}}>
                ...
                <Animated.View
                    style={{opacity: opacity, flex: 2, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between'}}
                >
                    <View style={{flexDirection: 'row', alignItems: 'center', flex: 3}}>
                        {this.props.playlist?.images[0]
                            ?
                                <Image source={{uri: this.props.playlist?.images[0]?.url}} style={{height: 40, width: 40, borderRadius: 10}} />
                            :
                                null
                        }
                        <Text numberOfLines={1} style={{color: 'white', marginLeft: 10, fontWeight: 'bold'}} >{this.props.playlist?.name}</Text>
                    </View>
                    <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                        <TouchableOpacity onPress={() => alert('liked')} >
                            <FontAwesome5Icon name='heart' size={24} color={'white'} />
                        </TouchableOpacity>
                        <TouchableOpacity onPress={() => alert('liked')} style={{marginLeft: 15}} >
