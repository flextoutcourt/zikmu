...
class Bigplayer extends React.PureComponent {
  ...
  _shuffle = () => {
    fetch(
      `https://api.spotify.com/v1/me/player/shuffle?state=${!this.props.store
        .listening.listening?.shuffle_state}`,
      {
        headers: {
          Accept: 'application/json',
          Authorization:
    ...
    this.state.is_playing ? (
      <TouchableOpacity onPress={() => this._determine_play_pause()}>
        <Icon
          ...
          color={'white'}
          size={24}
          style={styles.player.controls.icons.right}
        />
      </TouchableOpacity>
    ) : (
      <TouchableOpacity onPress={() => this._determine_play_pause()}>
  ...
  render() {
    return (
      <View style={{flex: 1}}>
        <SharedElement
          id={this.props.store.listening?.listening?.item?.album?.id}>
          <Animated.Image
            source={{
