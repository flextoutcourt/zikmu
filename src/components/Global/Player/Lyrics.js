...
function Lyrics({track}) {
  ...
  const _get_lyrics = (lt = lastTrack) => {
    if (first) {
      ...
      axios
        .get(
          `https://api.musixmatch.com/ws/1.1/matcher.lyrics.get?q_track=${track?.name}&q_artist=${track?.artists[0].name}&apikey=e0d2458b91da40316ebf92ac27cde04a`,
        )
        .then(data => {
          setLyrics(data.data.message.body);
          console.log(
            data.data.message.body.lyrics.pixel_tracking_url,
            data.data.message.body.lyrics.script_tracking_url,
          );
        });
      setFirst(false);
    } else if (
      lt?.name != track?.name ||
      lt?.artists[0]?.name != track?.artists[0]?.name
    ) {
      console.log('processed');
      axios
        .get(
