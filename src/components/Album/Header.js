            ...
            <Animated.View style={{flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-start', backgroundColor: Animated.color(0, 0, 0, background), marginTop: StatusBar.currentHeight, width: Dimensions.get('screen').width, padding: 10, zIndex: 99}}>
                ...
                >
                    ...
                    {this.props.album?.images[0]
                        ?
                            <Image source={{uri: this.props.album?.images[0]?.url}} style={{height: 40, width: 40, borderRadius: 10}} />
                        :
                            null
                    }
                    <Text numberOfLines={1} style={{color: 'white', marginLeft: 10, fontWeight: 'bold'}} >{this.props.album?.name}</Text>
                    </View>
                    <View>
